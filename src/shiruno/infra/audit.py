...
there is no parameter here that could carry a password, JWT, API key,
document content, embedding, or system prompt, so a caller cannot
accidentally log one even by mistake; this is a structural guarantee, not
just a convention callers are trusted to follow. Chat questions are
likewise never routed through this module at all — auditing only ever
covers administrator actions (login, upload, delete), not visitor
messages.
"""
...
def log_audit_event(
    ...
    *,
    ...
    _audit_logger.info(
        "audit action=%s outcome=%s administrator_id=%s username=%s document_id=%s",
        action,
        outcome,
        administrator_id,
        username,
